...
import (
	...
	GoIM_sdk "GoIM/pkg/proto/sdk_ws"
	"GoIM/pkg/utils"
	"context"
	"errors"
	"fmt"
...
func (d *DataBases) ReplaceMsgByIndex(suffixUserID string, msg *GoIM_sdk.MsgData, operationID string, seqIndex int) error {
...
func (d *DataBases) ReplaceMsgBySeq(uid string, msg *GoIM_sdk.MsgData, operationID string) error {
...
func (d *DataBases) GetMsgBySeqList(uid string, seqList []uint32, operationID string) (seqMsg []*GoIM_sdk.MsgData, err error) {
...
func (d *DataBases) GetMsgBySeqListMongo2(uid string, seqList []uint32, operationID string) (seqMsg []*GoIM_sdk.MsgData, err error) {
	var hasSeqList []uint32
	singleCount := 0
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
...
func (d *DataBases) GetSuperGroupMsgBySeqListMongo(groupID string, seqList []uint32, operationID string) (seqMsg []*GoIM_sdk.MsgData, err error) {
	var hasSeqList []uint32
	singleCount := 0
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
...
func (d *DataBases) GetMsgAndIndexBySeqListInOneMongo2(suffixUserID string, seqList []uint32, operationID string) (seqMsg []*GoIM_sdk.MsgData, indexList []int, unexistSeqList []uint32, err error) {
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
	sChat := UserChat{}
...
func genExceptionMessageBySeqList(seqList []uint32) (exceptionMsg []*GoIM_sdk.MsgData) {
...
func genExceptionSuperGroupMessageBySeqList(seqList []uint32, groupID string) (exceptionMsg []*GoIM_sdk.MsgData) {
	for _, v := range seqList {
		msg := new(GoIM_sdk.MsgData)
		msg.Seq = v
		msg.GroupID = groupID
